...
// Per Constitution Principle IV (Integration Testing) and spec.md Digest integration contract; P-F — Digest integration framework verified independently; Digest-side (`node-cron` + `Resend`) remains separate (`specs/research.md`); REST/JSON API contract provides integration mechanism (`contracts/notes-api.yaml`); `.env` DATABASE_URL variable/template (P-D production-style) — DB connection unavailable in environment (no PostgreSQL instance at `localhost:5432`); `.env` OAuth settings (`OAUTH_PROVIDER`, `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`) templates only — `next-auth` package not installed (`P-C` framework adopted); Digest-side scheduling (`node-cron`), ranking (`SC-006` — `featured` flag), selection cap (15 notes — contract assumption), generation (`Handlebars`), email delivery (`Resend`), retry/failure handling, subscription/user management remain Digest-side responsibilities — not implemented in Notes App repo; framework verifies Notes App REST contract readiness for Digest consumption independently
